...
# Set paths
filelist = os.path.join(StanfordIndoorDataPath, 'meta/area6_data_label.txt')
data_label_files = [os.path.join(StanfordIndoorDataPath, 'annotation/', line.rstrip()) for line in open(filelist)]
output_dir = os.path.join('indoor3d_ins_seg_hdf5')
...
for i in range(0, len(data_label_files)):
    ...
    if not os.path.exists(data_label_filename):
        continue
    data, label, inslabel = indoor3d_util.room2blocks_wrapper_normalized(data_label_filename, NUM_POINT, block_size=100.0, stride=20,
